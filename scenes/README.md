...
Run `python3 scripts/ppm_to_png.py` any time a `.png` preview above is missing or stale (e.g. after re-rendering a `.ppm`, or after a fresh clone if the PNGs weren't committed) — it writes `scenes/pngs/<name>.png` from each matching `scenes/*.ppm` (incomplete leftover PPMs are skipped with a warning). Pass an explicit path to convert one file only, e.g. `python3 scripts/ppm_to_png.py scenes/scene3_lowres.ppm`. Note on `.gitignore`: `*.ppm` is ignored repo-wide except under `scenes/` and `textures/` (both explicitly un-ignored); `.png` has no matching rule at all, so previews under `scenes/pngs/` are ordinary trackable files and only need regenerating if they're actually missing from your working copy.
