...
mod tests {
    use super::*;
    use crate::light::SCENE1_LIGHT_INTENSITY;
    ...
    fn scene1_uses_bright_key_light() {
        let (scene, _) = scene1_sphere(4.0 / 3.0);
        assert!(
            (scene.lights[0].intensity() - SCENE1_LIGHT_INTENSITY).abs() < 1e-12
        );
    }
    ...
    fn scene1_ground_shadow_darker_than_open_ground() {
        let (scene, _) = scene1_sphere(4.0 / 3.0);
        // Side-on rays onto the floor: under the sphere vs off to +X.
        let under = trace(
            &scene,
